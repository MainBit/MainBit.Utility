...
namespace MainBit.Utility.Taxonomies.Projections
{
    [OrchardFeature("MainBit.Utility.Taxonomies")]
    public class TermsFilter : IFilterProvider
    {
        ...
        public void Describe(dynamic describe)
        {
            describe.For("Taxonomy", T("Taxonomy"), T("Taxonomy"))
                .Element("TokenizedHasTerms", T("Tokenized Has Terms"), T("Categorized content items"),
                    (Action<dynamic>)ApplyFilter,
                    (Func<dynamic, LocalizedString>)DisplayFilter,
                    "TokenizedSelectTerms"
                );
        }
        ...
        public void ApplyFilter(dynamic context)
        {
            ...
            var listTermIds = new List<string> { (string)context.State.TermIds, (string)context.State.TermTokens };
            var termIds = String.Join(",", listTermIds.Where(p => !string.IsNullOrEmpty(p)));
